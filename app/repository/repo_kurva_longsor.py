...
def get_reference_curves_longsor():
    """Mengambil referensi kurva Longsor dari database dengan transaksi yang aman."""
    try:
        ...
        with db.session.no_autoflush:  # Menghindari nested transaction error
            curves = db.session.query(LongsorReferenceCurve).all()
        ...
        logger.error(f"❌ ERROR: Gagal mengambil referensi kurva Longsor dari database: {e}")
